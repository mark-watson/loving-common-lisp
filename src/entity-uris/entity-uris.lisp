;; Copyright Mark Watson 2019-2022. All Rights Reserved.   http://markwatson.com
;; License: AGPL version 3 (http://www.gnu.org/licenses/agpl-3.0.txt)

(in-package :entity-uris)
...
(print a1)
(print (length a1))
(defvar *current-directory* (subseq a1 3 (- (length a1) 18)))
...
(defun ensure-uri-brackets (a-uri)
  (if (equal (type-of a-uri) 'cons)
    (setf a-uri (cadr a-uri)))
  (if (string-equal (subseq a-uri 0 1) "<")
      a-uri
    (concatenate 'string "<" a-uri ">")))
;;
  ...
  (let* ((len (length words))
    ...
    (dotimes (i len)
	  ...
	  (let* ((4-words (concatenate 'string (aref words i) " " (aref words (+ i 1)) " "
	    ...
	    (if a-uri
		(setq ret (cons (list 4-words (ensure-uri-brackets a-uri)) ret)))))
      ;; 3 word entities:
      (if (< i (- len 2))
	  (let* ((3-words (concatenate 'string (aref words i) " " (aref words (+ i 1))
	    ...
	    (if a-uri
		(setq ret (cons (list 3-words (ensure-uri-brackets a-uri)) ret)))))
      ;; 2 word entities:
      (if (< i (- len 1))
	  (let* ((2-words (concatenate 'string (aref words i) " " (aref words (+ i 1))))
		 (a-uri (gethash 2-words entity-hash)))
	    (if a-uri
		(setq ret (cons (list 2-words (ensure-uri-brackets a-uri)) ret)))))
      ;; 1 word entity:
