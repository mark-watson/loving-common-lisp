# AutoContext: Prepare Effective Prompts and Context for LLM Queries
...
This listing of **main.lisp**, the core of our hybrid Retrieval-Augmented Generation (RAG) system written in Common Lisp that also constructs a one-shot prompt for later input into a LLM.
...
Normally you would combine the code developed in this chapter in an application using a LLM like Gemini-2.5-pro or GPT-5. Here, I feed the generated prompt into a small 3B model running on Ollama for test purposes (most output not shown):
...
My motivate for writing this example code is my desire to mostly write applications using smaller LLMs running locally using Ollama or LM Studio. Often these small models only support a context size from 16K to 64K in size and they run slowly on my laptop.
